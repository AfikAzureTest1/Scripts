...
#	* Make a file containing a list with all the nodes and their IPs, in the following format:
#		<server_name_1> <server_ip_1>
#		<server_name_2> <server_ip_2>
#		<server_name_3> <server_ip_3>
...
#	* Create an LVM mount from a specified disk for the elasticsearch data directory.
...
# You may (and should) change the nodes information variables. You can also change the default definition for the configuration.

# Script exit codes:
# 1 - Script run as non root.
# 2 - Failed to split of the servers_info (names an IPs) failed.
# 3 - Failed to install Java.
# 4 - Failed to import GPG-KEY for elasticsearch repository.
# 5 - Failed to install Elastic.
# 6 - Failed to edit /etc/hosts.
# 7 - Failed to start Elastic.
# 8 - Failed to enable Elastic to start automatically on boot.
# 9 - Failed to create a firewall rule.
# 10 - Failed to reload firewall changes.
# 11 - Failed to create the elasticsearch repo file.
...
#####	You can change this variables section - Start	######

# Variables default definition for the configuration.
...
declare servers_info_file_path='/tmp/es-nodes-list.txt'	# List of all nodes in the format '<server_name> <server_ip>'
...
# Split the nodes names and ips to 2 different files.
...
cut -d ' ' -f 1 $servers_info_file_path > /tmp/AfikArbivNodesNameList.txt
...
cut -d ' ' -f 2 $servers_info_file_path > /tmp/AfikArbivNodesIPList.txt

# Checking that the IPs file does not contain any alpha-chars.
if [ $(cat /tmp/AfikArbivNodesIPList.txt | grep -E [a-Z] -c) -ne 0 ]; then
	echo_fail "The split of the servers_info_file failed. (The IP list after the split contains alpha-char(s)."
	echo_fail "Please verify that the servers_info_file is in the correct format."
	echo_fail "The format should be '<server_name> <server_ip>'"
...
grep -Ff /tmp/AfikArbivNodesNameList.txt /etc/hosts | grep -v ${HOSTNAME} | grep -E -o "([0-9]{1,3}[\.]){3}[0-9]{1,3}" > /tmp/AfikArbivNodesIPListWithoutMe.txt
if [ $? -ne 0 ]; then
